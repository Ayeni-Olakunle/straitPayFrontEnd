    ...
    <section>
        ...
        {toggle === true && <div>
            ...
            <div className="flex justify-start items-center gap-[20px] text-[17px] text-[#8D8D8D]">
              <IoIosCheckbox className="cursor-pointer text-xl"/>
              <p>{item.todoName}</p>
            </div>
        ...
        {toggle === false && <div>
            {data.map((item, index) => {
                ...
                <div className="flex justify-start items-center gap-[20px] text-[17px] text-[#8D8D8D]">
                <IoIosCheckbox className="cursor-pointer text-xl text-[#407bff]"/>
                <p className="line-through">{item.todoName}</p>
                </div>

                <div className="flex justify-between items-center gap-[15px] text-[17px]">
                <FaEye className="text-[#66C566] cursor-pointer text-xl" onClick={() => handleModal(true)}/>
                <IoClose className="text-[#FF6347] text-[22px] cursor-pointer" onClick={() => handleDelete(item._id)} />
                </div>
            </div>
                    )
                }
        ...
        {/* Details Modal */}
        {modal && <div className="absolute top-[0] w-full left-[0] h-screen flex justify-center bg-[#000000a6] overflow-y-scroll items-start">
            <div className="w-[450px] bg-[white] h-auto p-[20px] rounded-[5px] mx-[0] my-[40px]">
                ...
                <div className="w-full flex justify-start items-start flex-col mx-[0] my-[10px]">
                    ...
                    <input type="text" id="title" name="title" placeholder="Title" value={modalData?.todoName} className="border-[1.4px] border-solid border-[#afafaf] w-full p-[10px] text-[17px] rounded-[5px] mx-[0] my-[5px] outline-none bg-[#F6F6F6] text-[#606060]" />
                </div>
            <div className="w-full flex justify-start items-start flex-col mx-[0] my-[10px]">
                <label htmlFor="link" className="text-[#818181] text-lg w-full outline-none">Link</label>
                <input type="url" id="link" name="link" placeholder="Link" value={modalData?.todoLink} className="border-[1.4px] border-solid border-[#afafaf] w-full p-[10px] text-[17px] rounded-[5px] mx-[0] my-[5px] outline-none bg-[#F6F6F6] text-[#606060]" />
            </div>
            <div className="flex justify-between items-center gap-[50px]">
                <div className="w-full flex justify-start items-start flex-col mx-[0] my-[10px]">
                    <label htmlFor="createdAT" className="text-[#818181] text-lg w-full outline-none">Created Date</label>
                    <input type="date" id="createdAT" name="createdAT" value={timeStampDate(modalData?.createdAt)} placeholder="Create" className="border-[1.4px] border-solid border-[#afafaf] w-full p-[10px] text-[17px] rounded-[5px] mx-[0] my-[5px] outline-none bg-[#F6F6F6] text-[#606060]" disabled />
                </div>
                <div className="w-full flex justify-start items-start flex-col mx-[0] my-[10px]">
                    <label htmlFor="update" className="text-[#818181] text-lg w-full outline-none">Update Date</label>
                    <input type="date" id="update" name="update" value={timeStampDate(modalData?.updateAt)} placeholder="update" className="border-[1.4px] border-solid border-[#afafaf] w-full p-[10px] text-[17px] rounded-[5px] mx-[0] my-[5px] outline-none bg-[#F6F6F6] text-[#606060]" disabled />
                </div>
            </div>
            <div className="w-full flex justify-start items-start flex-col mx-[0] my-[10px]">
                <label htmlFor="desc" className="text-[#818181] text-lg w-full outline-none">Description</label>
                <textarea name="desc" id="desc" rows="3" placeholder="Description" value={modalData?.todoDescription} className="border-[1.4px] border-solid bg-[#F6F6F6] border-[#afafaf] w-full p-[10px] text-[17px] rounded-[5px] mx-[0] my-[5px] outline-none text-[#606060]" ></textarea>
            </div>
            <div className="w-full flex justify-between items-end mx-[0] my-[10px]">
                ...
                <button type="submit" className="bg-[#407BFF] text-lg font-bold px-[25px] py-[10px] text-[white] rounded-[5px]">Update</button>
            </div>
            </div>
        </div>}
        <ToastContainer />
